...
def preprocess_data(df):
    ...
    # Remove columns with too many NaN values for Churn = 0
    df = remove_columns(df, threshold=0.3)
...
def display_feature_importances(rf, X_cleaned):
    ...
    print(f"Top {num_feature} most important features:")
    ...
    plt.barh(range(num_feature), importances[indices[:num_feature]], align="center")
    plt.yticks(range(num_feature), [X_cleaned.columns[i] for i in indices[:num_feature]])
